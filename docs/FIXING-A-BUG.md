...
| **A property the test's own interface cannot reach** | An end-to-end test said it proved coding is retried after being abandoned — "which only happens if coding is periodically retried rather than abandoned for good". It could not: `seal` codes any payload too large to send raw *whatever* the heuristic decided (`must_try`), and the test's payloads were all oversized. Breaking the retry outright left it passing. Before believing a comment like that, disable the mechanism it names and watch. Where the property is not observable through the interface under test — this one is a sender-side decision a receiver cannot see — the test belongs next to the decision, not at the far end of a socket. |

| **A stimulus the test never confirms arrived** | `a_replayed_opening_frame_does_not_displace_the_session_it_names` sent one captured frame and asserted the session still worked. It checked that the replay was *sent*, never that it was *received* — and it cannot, because refusing a replay correctly means answering with silence, so a replay lost on loopback is indistinguishable from one ignored. Removing the protection outright left the test passing two runs in six. Ten replays instead of one, and the assertion moved to what the server records — a second session would be a second `Connected` — catches it twenty times in twenty. A test whose stimulus can vanish is a test that sometimes runs no experiment; make the stimulus reliable, and assert on the thing that only the defect could change. |
