...
pub(crate) enum Ingested {
    /// The frame did not authenticate: forged, replayed, or misdirected.
    ///
    /// Separate from [`Ingested::Nothing`] because some decisions turn on
    /// whether the sender held the key, and anyone at all can send bytes to a
    /// UDP port. Treating the two alike is how a caller ends up crediting an
    /// attacker for traffic it only had to address correctly.
    Rejected,
    /// Authenticated, with nothing for the application: an acknowledgement, a
    /// duplicate, or a frame handled entirely within the session layer.
    Nothing,
    ...
    /// A peer proved it can receive where a challenge was sent, echoing the
    /// token back.
    ///
    /// Which address that was is the caller's business — this layer never sees
    /// one — so the token is handed up for the caller to match against the
    /// probe it started.
    PathValidated([u8; PATH_TOKEN_LEN]),
...
impl Peer {
    ...
    /// Builds a challenge asking the peer to prove it can receive somewhere.
    ///
    /// The token is generated by the caller, which is also the only place that
    /// knows which address is being probed.
    pub fn challenge(&mut self, token: &[u8; PATH_TOKEN_LEN], out: &mut [u8]) -> Result<usize> {
        Ok(self.session.seal_path_challenge(token, out)?)
    }

    /// Discards what was measured about the path this session used to be on.
    pub fn forget_path(&mut self) {
        self.retransmit.forget_path();
    }
    ...
    pub fn ingest(
        ...
        *ack_len = 0;
        ...
        let opened: Opened = match self.session.open(frame) {
            ...
            // a denial of service. It is still reported as distinct from a
            // frame that authenticated and carried nothing.
            Err(_) => return Ok(Ingested::Rejected),
        };

        // A challenge is answered where it arrived from, which is the whole
        // of what it proves. `ack` is the caller's reply buffer and every
        // caller already sends it back to the source, so both front ends
        // answer without knowing this frame type exists.
        if opened.header.frame_type == FrameType::PathChallenge {
            if opened.len != PATH_TOKEN_LEN {
                return Ok(Ingested::Nothing);
            }
            let mut token = [0u8; PATH_TOKEN_LEN];
            token.copy_from_slice(&frame[HEADER_LEN..HEADER_LEN + PATH_TOKEN_LEN]);
            *ack_len = self.session.seal_path_response(&token, ack)?;
            return Ok(Ingested::Nothing);
        }

        if opened.header.frame_type == FrameType::PathResponse {
            if opened.len != PATH_TOKEN_LEN {
                return Ok(Ingested::Nothing);
            }
            let mut token = [0u8; PATH_TOKEN_LEN];
            token.copy_from_slice(&frame[HEADER_LEN..HEADER_LEN + PATH_TOKEN_LEN]);
            return Ok(Ingested::PathValidated(token));
        }
