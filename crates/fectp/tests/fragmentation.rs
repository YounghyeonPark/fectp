...
fn an_empty_message_survives_the_round_trip() {
    ...
    client
        .send_reliable(&[], PayloadType::Opaque)
        .and_then(|()| client.flush(TIMEOUT))
        .expect("send_reliable");
...
fn a_message_above_the_reassembly_ceiling_is_refused() {
    ...
    assert!(
        client
            .send_reliable(&huge, PayloadType::Opaque)
            .and_then(|()| client.flush(TIMEOUT))
            .is_err(),
...
fn ordinary_sends_are_unaffected_by_a_fragmented_one() {
    ...
    client
        .send_reliable(&big, PayloadType::Opaque)
        .and_then(|()| client.flush(TIMEOUT))
        .expect("send_reliable");
...
fn two_fragmented_messages_do_not_mix() {
    ...
    client
        .send_reliable(&first, PayloadType::Opaque)
        .and_then(|()| client.flush(TIMEOUT))
        .expect("first");
    client
        .send_reliable(&second, PayloadType::Opaque)
        .and_then(|()| client.flush(TIMEOUT))
        .expect("second");
