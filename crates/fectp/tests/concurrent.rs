...
fn sending_works_while_another_thread_is_blocked_reading() {
    ...
    std::thread::scope(|s| {
        ...
        conn.send(b"sent while the reader waits", PayloadType::Opaque)
            .expect("send");
...
fn both_directions_run_at_once() {
    ...
    std::thread::scope(|s| {
        s.spawn(|| {
            for i in 0..MESSAGES {
                conn.send(&(i as u32).to_le_bytes(), PayloadType::Opaque)
                    .expect("send");
            }
        });
...
fn a_blocked_read_does_not_hold_up_a_send() {
    ...
    std::thread::scope(|s| {
        ...
        conn.send(b"not waiting for the reader", PayloadType::Opaque)
            .expect("send");
